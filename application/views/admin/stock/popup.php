...
// Form open
echo form_open_multipart(base_url('admin/popup/edit/'.$stock->id_galeri));
?>
...
<label>Status popup</label>
  <select name="popup_status" class="form-control">
    ...
    <option value="Draft" <?php if($stock->popup_status=="Draft") { echo "selected"; } ?>>Non Aktif</option>
  </select>
</div>
...
<input type="url" name="website" class="form-control" placeholder="http://website.com" value="<?php echo $stock->website ?>">
</div>
</div>
